...
power curve: the primary estimator's jackknife test has zero power against any effect
up to −12% (its SE is 1.7-6.5× the true sampling dispersion), and with correctly
calibrated inference the minimum detectable effect at 80% power ranges from 4.5%
(Callaway-Sant'Anna) to 6.4% (primary). Effects in the low single digits, where the
...
notebook_src/                Python modules used by the notebook: data prep,
                             figures, results memo, paper table/macro generation
...
(`SEED = 20260524`); step runtimes are logged to `Results/runtimes.csv`.
...
and in-text number is generated by `notebook_src/make_tables.py` from `Results/csv/`.
...
- Inference: jackknife / placebo / bootstrap per estimator, prediction intervals
  (Cattaneo–Feng–Titiunik 2021), pooled randomization inference, and a design-based
  power simulation with a pre-stated compute budget.
